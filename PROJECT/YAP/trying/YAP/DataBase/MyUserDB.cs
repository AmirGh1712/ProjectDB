...
namespace YAP.DataBase
{
    public class MyUserDB : IUserDB
    {
        ...
        public async IAsyncEnumerable<Place> GetRecommendedPlaces(string uname)
        {
            ...
            using MySqlCommand command = new MySqlCommand("SELECT *, avgstars as stars FROM(SELECT places.idPlaces, AVG(stars) AS avgstars FROM places INNER JOIN reviews ON places.idPlaces = reviews.idPlaces WHERE category IN(SELECT category FROM(SELECT category, MAX(stars) FROM(SELECT category, COALESCE(AVG(stars), -1) AS stars FROM reviews JOIN places ON places.idPlaces = reviews.idPlaces WHERE username = BINARY @username GROUP BY category) AS tbl) AS tbl2) GROUP BY places.idPlaces) AS tbl3 INNER JOIN places ON places.idPlaces = tbl3.idPlaces ORDER BY avgstars DESC, RAND() LIMIT 5;", con);
            command.Parameters.AddWithValue("@username", uname);
            using MySqlDataReader rdr = (MySqlDataReader)await command.ExecuteReaderAsync();
